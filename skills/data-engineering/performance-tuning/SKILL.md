...
For distributed engines (Spark and similar), the bottleneck is usually the
shuffle, partition counts, and skewed stragglers rather than the SQL-shaped
fixes above — see `spark-performance` if installed.
...
query plan (full scans, skew, exploding joins, stale stats), the
high-leverage fixes in priority order, and the anti-patterns — see
[TUNING-CHECKLIST.md](./TUNING-CHECKLIST.md). The principles are
tool-agnostic; map feature names to your engine (consult
