...
it; don't let chat and email disagree. Use UTC timestamps. Chat carries the
live heartbeat; **email/broadcast** reaches stakeholders not in the channel
(execs, downstream consumers) — one stable thread per incident, same facts,
fuller framing (subject convention + email templates in POSTMORTEM-TEMPLATE.md).
...
For fill-in templates — the severity matrix (`docs/agents/incident-severity.md`),
the status-update messages, and the blameless post-incident review — see
[POSTMORTEM-TEMPLATE.md](./POSTMORTEM-TEMPLATE.md). The review structure
follows [Google SRE's blameless postmortem practice](https://sre.google/sre-book/postmortem-culture/):
